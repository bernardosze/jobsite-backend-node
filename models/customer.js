...
const Customer = sequelize.define(
	'customers',
	{
		...
		name: {
			...
			allowNull: false,
			validate: {
				notEmpty: { msg: 'Customer Name must be provided.' },
				len: { args: [3, 45], msg: 'Customer Name length must be between 3 and 45 characters.' }
			}
		},
		address: {
			type: Sequelize.STRING,
			...
			validate: {
				...
				max: { args: [100], msg: 'Address cannot have more than 100 characters.' }
			}
		},
		city: {
			type: Sequelize.STRING,
		...
		phone: {
			...
			allowNull: false,
			validate: {
				notEmpty: { msg: 'Customer phone must be provided.' },
				isNumeric: true,
				len: {
					args: [11, 15],
					msg: 'Customer Phone number must have at least 11 numeric characters.'
				}
			}
		},
		email: {
